        ...
        <div style="font-family: Georgia, serif; max-width: 520px; margin: 0 auto; background: #0f0a0c; color: #f5e8ee; padding: 40px 32px;">
          ...
          <div style="background: #1a1015; border: 1px solid #4d2a3d; padding: 28px; margin-bottom: 28px; position: relative;">
            ...
            <p style="font-size: 14px; color: #9a7080; line-height: 1.8; margin-bottom: 20px;">
              You have completed <strong style="color: #e8839a;">5 bookings</strong> with Acrylic Faerie and you have earned your loyalty reward!
            </p>
            ...
            <p style="font-size: 13px; color: #9a7080; line-height: 1.8;">Simply mention this email at your next appointment and we will apply your discount automatically. No code needed! ✦</p>
          </div>
          <div style="background: #1a1015; border: 1px solid #3a1f2e; padding: 20px; margin-bottom: 28px;">
            ...
            <p style="font-size: 13px; color: #9a7080; line-height: 1.8;">✦ Book at <a href="https://acrylicfaerie.com" style="color: #e8839a;">acrylicfaerie.com</a></p>
          ...
          <p style="text-align: center; font-size: 11px; color: #5a3a48; margin-top: 28px;">Questions? <a href="mailto:acrylicfaerie.biz@gmail.com" style="color: #7a2840;">acrylicfaerie.biz@gmail.com</a></p>
        </div>
      `,
    }),
  });
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (rewardsEarned > loyaltySent) {
      const { ok, data } = await sendLoyaltyEmail(email, name || email.split("@")[0]);
      if (ok) {
        await fetch(`${SUPABASE_URL}/rest/v1/clients?id=eq.${client.id}`, {
          method: "PATCH",
          headers: {
