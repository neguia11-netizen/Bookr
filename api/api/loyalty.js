...
const RESEND_API_KEY = "re_V4iUXzeR_AqkXJnsHKrniRQk8uBMrUWSw";
...
async function incrementBookingCount(clientId) {
  const res = await fetch(`${SUPABASE_URL}/rest/v1/clients?id=eq.${clientId}`, {
    ...
    body: JSON.stringify({ booking_count: "booking_count + 1" }),
  });
  // Use RPC for increment instead
  const rpcRes = await fetch(`${SUPABASE_URL}/rest/v1/rpc/increment_booking_count`, {
    method: "POST",
    headers: {
      "apikey": SUPABASE_KEY,
      "Authorization": `Bearer ${SUPABASE_KEY}`,
      ...
      html: `
        <div style="font-family: Georgia, serif; max-width: 520px; margin: 0 auto; background: #0f0a0c; color: #f5e8ee; padding: 40px 32px;">
          ...
          <div style="background: #1a1015; border: 1px solid #4d2a3d; padding: 28px; margin-bottom: 28px; position: relative;">
            ...
            <p style="font-size: 15px; color: #f5e8ee; line-height: 1.8; margin-bottom: 16px;">
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Increment booking count directly via SQL
    await fetch(`${SUPABASE_URL}/rest/v1/clients?id=eq.${client.id}`, {
      ...
      body: JSON.stringify({ booking_count: (client.booking_count || 0) + 1 }),
    });

    const newCount = (client.booking_count || 0) + 1;
    const loyaltySent = client.loyalty_emails_sent || 0;

    // Check if they've hit a multiple of 5 and haven't received this reward yet
